    ...
    for x = 0.5:0.5:cap-0.5
        ...
        for d = 1:numcalc_dzq
            for z = 1:numcalc_dzq
                for q = 1:numcalc_dzq
                    for qp = 0:cap
                        sumtotal = sumtotal + d*eq(d,z-cap)*eq(q,z)*poisspdf(z,x)*pj(qp+1); 
                    end
                    for qp = (cap+1):(2*cap-1)
                        sumtotal = sumtotal + d*eq(d,qp+z-(2*cap))*eq(q,qp-cap+z)*poisspdf(z,x)*pj(qp+1);
                    end
                    for qp = (2*cap):numcalc
                        sumtotal = sumtotal + d*eq(d,z)*eq(q,qp-cap+z)*poisspdf(z,x)*pj(qp+1);
                    end
                end
            end
        end
