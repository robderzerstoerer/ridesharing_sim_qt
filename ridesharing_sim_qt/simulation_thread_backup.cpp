if (par.number_of_bus_calculations > 1)
{
	...
	for (double iter_B = index_low_B; iter_B <= (index_high_B + 0.0001); iter_B += (index_high_B - index_low_B) / (par.number_of_bus_calculations - 1))
	{
		...
		{
			double index_low_x = par.normalized_request_rate_from;
			double index_high_x = par.normalized_request_rate_to;
			for (double iter_x = index_low_x; iter_x <= (index_high_x + 0.0001); iter_x += (index_high_x - index_low_x) / (par.number_of_request_rates - 1))
			{
				if (par.stop_thread)
					break;
				double normalized_request_rate = iter_x;
		...
		{
			sim.init_new_sim(number_of_buses, par.normalized_request_rate, std::max((ULL)50000, 50 * number_of_buses));
			...
			if (par.simulate_until_exact)
			{
				...
				while (!exact_result)
				{
					...
					//simulate (and measure) for 100 requests per bus (at least 100000 requests)
					sim.run_sim_requests(std::max((ULL)50000, 100 * number_of_buses));
...
else if (par.number_of_request_rates > 1)
{
	...
	double index_low = par.normalized_request_rate_from;
	double index_high = par.normalized_request_rate_to;
	for (double iter = index_low; iter <= (index_high + 0.0001); iter += (index_high - index_low) / (par.number_of_request_rates - 1))
	{
		if (par.stop_thread)
			break;
		double normalized_request_rate = iter;
